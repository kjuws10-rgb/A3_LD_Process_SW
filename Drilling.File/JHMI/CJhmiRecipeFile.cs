...
public sealed class CJhmiRecipeFile(string configRoot) : IRecipeFile
{
    ...
    public Task Save(ST_RECIPE_DATA recipe, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var parameter in cellScopedParameters)
        {
            ...
            if (recipeExists)
            {
                var oldValue = GetCellScopedValue(
                    oldValues,
                    parameter.Key,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var legacyValue in ReadRecipeValues(oldPath))
        {
            if (!TryGetLegacyCellParameter(legacyValue.Name, out var cellNo, out var parameterName) ||
                cellNo > activeCellCount)
            {
                continue;
            }

            var scopedKey = $"CELL{cellNo}_{parameterName}";
            var persistedKey = ToPersistedCellScopedKey(scopedKey);
            var expectedKey = CreateKey("CELL", persistedKey);
            if (expectedValues.ContainsKey(expectedKey))
            {
                continue;
            }

            expectedValues[expectedKey] = legacyValue.Value;
            lines.Add(FormatRecipeLine("CELL", persistedKey, legacyValue.Value));
        }
    ...
    private ST_RECIPE_DATA? LoadRecipe(string path, IReadOnlyList<ST_RECIPE_FORM_ITEM> formItems)
    {
        ...
        foreach (var legacyValue in ReadRecipeValues(path))
        {
            if (!TryGetLegacyCellParameter(legacyValue.Name, out var cellNo, out var parameterName) ||
                !formItemsByName.TryGetValue($"CELL_{parameterName}", out var template))
            {
                continue;
            }

            var scopedKey = $"CELL{cellNo}_{parameterName}";
            if (parameters.Any(parameter => parameter.Key.Equals(scopedKey, StringComparison.OrdinalIgnoreCase)))
            {
                continue;
            }
    ...
    private static bool TryGetLegacyCellParameter(string key, out int cellNo, out string parameterName)
    {
        cellNo = 0;
        parameterName = "";
        ...
        if (separatorIndex <= 4 ||
            !int.TryParse(key.AsSpan(4, separatorIndex - 4), NumberStyles.None, CultureInfo.InvariantCulture, out cellNo))
        {
            return false;
        }

        parameterName = key[(separatorIndex + 1)..];
    ...
    private static int GetActiveCellCount(IEnumerable<ST_RECIPE_PARAM> parameters)
    {
        ...
        {
            countValue = parameters.FirstOrDefault(parameter =>
                parameter.Key.Equals("MAX_CELL_NUMBER", StringComparison.OrdinalIgnoreCase))?.Value;
        ...
        IReadOnlyList<ST_RECIPE_PARAM> parameters)
    {
        ...
        if (!TryGetHoleOverrideMetadata(parameterName, out var holeNo, out _, out _, out _, out _))
        {
            return true;
        }
        ...
        return holeNo <= (long)countX * countY;
    }
    ...
    private static int GetActiveCellCount(IReadOnlyDictionary<string, string> values)
    {
        ...
        if (!int.TryParse(countValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out var count))
        {
            countValue = GetValue(values, "PROCESS", "MAX_CELL_NUMBER", "1");
            int.TryParse(countValue, NumberStyles.Integer, CultureInfo.InvariantCulture, out count);
        }
    ...
    {
        var dotIndex = key.IndexOf('.', 4);
        var underscoreIndex = key.IndexOf('_', 4);
        if (dotIndex < 0)
        {
            return underscoreIndex;
        }

        return underscoreIndex < 0 ? dotIndex : Math.Min(dotIndex, underscoreIndex);
    }

    private static string NormalizeCellScopedParameterName(string parameterName)
    {
        var normalized = parameterName.StartsWith("CELL_", StringComparison.OrdinalIgnoreCase)
            ? parameterName[5..]
            : parameterName;

        if (!normalized.StartsWith("HOLE", StringComparison.OrdinalIgnoreCase))
        {
            return normalized;
        }

        var separatorIndex = 4;
        while (separatorIndex < normalized.Length && char.IsDigit(normalized[separatorIndex]))
        {
            separatorIndex++;
        }

        return separatorIndex > 4 && separatorIndex < normalized.Length - 1
            ? $"{normalized[..separatorIndex]}_{normalized[(separatorIndex + 1)..]}"
            : normalized;
    }
    ...
    private static string GetCellScopedValue(
        IReadOnlyDictionary<string, string> values,
        string key,
        string defaultValue)
    {
        var persistedKey = CreateKey("CELL", ToPersistedCellScopedKey(key));
        if (values.TryGetValue(persistedKey, out var value))
        {
            return value;
        }

        var legacyDottedKey = CreateKey("CELL", ToLegacyDottedCellScopedKey(key));
        return values.TryGetValue(legacyDottedKey, out value) ? value : defaultValue;
    }

    private static string ToLegacyDottedCellScopedKey(string key)
    {
        var cellNo = GetCellNo(key);
        var parameterName = GetCellParameterName(key);
        if (cellNo <= 0 || string.IsNullOrWhiteSpace(parameterName))
        {
            return key;
        }

        if (TryGetHoleOverrideMetadata(parameterName, out _, out _, out _, out _, out _))
        {
            var separatorIndex = 4;
            while (separatorIndex < parameterName.Length && char.IsDigit(parameterName[separatorIndex]))
            {
                separatorIndex++;
            }

            if (separatorIndex < parameterName.Length && parameterName[separatorIndex] == '_')
            {
                parameterName = $"{parameterName[..separatorIndex]}.{parameterName[(separatorIndex + 1)..]}";
            }
        }

        return $"CELL{cellNo}.{parameterName}";
    }
        ...
        IReadOnlyList<ST_RECIPE_PARAM> parameters)
    {
            ...
            out _))
        {
            var countXValue = parameters.FirstOrDefault(item =>
                item.Key.Equals($"CELL{cellNo}_NUM_OF_PIXEL_X", StringComparison.OrdinalIgnoreCase))?.Value;
            var countX = int.TryParse(
                countXValue,
                NumberStyles.Integer,
                CultureInfo.InvariantCulture,
                out var parsedCountX) && parsedCountX > 0
                    ? parsedCountX
                    : 1;
            var column = ((holeNo - 1) % countX) + 1;
            var row = ((holeNo - 1) / countX) + 1;
            return $"Cell{cellNo} {ToColumnLetter(column)}{row} {displayName}";
        }
    ...
    private static bool TryGetHoleOverrideMetadata(
        string parameterName,
        out int holeNo,
        ...
        out double max)
    {
        ...
        if (!parameterName.StartsWith("HOLE", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }

        var separatorIndex = parameterName.IndexOfAny(['_', '.'], 4);
        if (separatorIndex <= 4 ||
            !int.TryParse(
                parameterName.AsSpan(4, separatorIndex - 4),
                NumberStyles.None,
                CultureInfo.InvariantCulture,
                out holeNo) ||
            holeNo <= 0)
        ...
        var fieldName = parameterName[(separatorIndex + 1)..].ToUpperInvariant();
        switch (fieldName)
        {
            case "OFFSET_X":
