...
| No | Message | Direction | 목적 | 주요 Payload |
| ---: | --- | --- | --- | --- |
| 60 | `EQUIPMENT_GEOMETRY_SYNC_REQ` | Process → Review | H1 초기위치, Camera→Scanner 고정 물리 Offset과 Scanner 배치 Geometry 전달 | calibrationId, reviewCenter, firstScannerInitialStage, reviewToFirstScannerOffset, originTolerance, scannerPitch, evenYOffset, axisSign |
| 61 | `EQUIPMENT_GEOMETRY_SYNC_RSP` | Review → Process | Calibration 버전 및 값 일치 여부 응답 | accepted, calibrationId, checksum, mismatchFields |
| 62 | `MOTION_SEQUENCE_SYNC_REQ` | Process → Review | Stage 왕복 동작과 역방향 MOF/후측정 순서 전달 | homeStageY, forwardTransportSignY, turnaroundStageY, motionSteps[] |
| 63 | `MOTION_SEQUENCE_SYNC_RSP` | Review → Process | Review 후측정 위치 및 시퀀스 적용 가능 여부 응답 | accepted, reviewMeasureStageY, reason |
...
| `firstScannerInitialStageX/Y` | 장비 설계 또는 Teaching으로 확정한 H1 Scanner 실제 Stage 중심 |
| `reviewToFirstScannerOffsetX/Y` | Review Camera 중심에서 H1 Scanner 중심까지의 고정 물리 벡터 |
| `scannerOriginTolerance` | H1 실제 중심과 ReviewCenter+Offset 기대값의 허용오차 |
...
Motion Sequence 필수 규칙:

```text
Home → Review Camera 통과 → Scanner Turnaround → Reverse MOF
     → Review Camera 후측정 → Home

ForwardTransportSignY = +1이면 MofExecutionCommands는 ProcessStageY 내림차순
ForwardTransportSignY = -1이면 MofExecutionCommands는 ProcessStageY 오름차순
```
...
ExpectedH1 = ReviewCenter + ReviewToFirstScannerOffset
abs(FirstScannerInitialStage - ExpectedH1) <= ScannerOriginTolerance
```
...
| `mofSequence` | Scanner 뒤쪽에서 Review Camera 방향으로 돌아오는 역방향 MOF 실행 순번 |
...
| `reviewX` / `reviewY` | 실제 Review Camera 기준 좌표. ScannerRelativeStage + PhysicalOffset + SelectedDoeStageOffset |

`PROCESS_PLAN_DOWNLOAD_REQ`에는 다음 Motion 정보가 포함되어야 한다.

| Field | 설명 |
| --- | --- |
| `homeStageY` | 공정 시작/종료 원점 Y |
| `forwardTransportSignY` | Home→Review→Scanner 정물류 Y 부호 |
| `turnaroundStageY` | Scanner 뒤쪽에서 이동 방향을 반전할 Stage Y |
| `mofOnReverseTravel` | 역물류 복귀 중 MOF 수행 여부. 본 구조에서는 `true` |
| `postReviewStageY` | MOF 완료 후 가공 결과를 측정하는 Review Camera Stage Y |
| `motionSteps[]` | 전진, 반전, 역방향 MOF, Review 후측정, Home 복귀 단계 |
...
14. `MOTION_SEQUENCE_SYNC_REQ`
15. `MOTION_SEQUENCE_SYNC_RSP`
16. `DOE_CONFIG_SYNC_REQ`
17. `CELL_TARGET_LIST_DOWNLOAD_REQ`
18. `AK_MEASURE_REQ`
19. `AK_MEASURE_RESULT`
20. `REVIEW_MEASURE_REQ`
21. `REVIEW_MEASURE_PROGRESS`
22. `REVIEW_POINT_RESULT`
23. `REVIEW_MEASUREMENT_BATCH_RESULT`
24. `REVIEW_MEASURE_COMPLETED`
25. `REVIEW_MEASURE_FAILED`
26. `OFFSET_POLICY_SET_REQ`
27. `OFFSET_CALC_RSP`
28. `OFFSET_APPLY_REQ`

이 28개를 먼저 표준화하면 고정 설비 Geometry와 실제 왕복 Motion Sequence 확인부터 Process / Review 기본 운용, 측정, 결과 수신, 동적 보정 반영까지 한 사이클을 닫을 수 있다.
