...
4. 실행 환경을 선택한다. `Simulation - Virtual Wait`는 Virtual 축만 허용하고, `Equipment - Hardware Coordinate`는 설비 확인 4개 항목을 모두 요구한다.
...
| `required axes` 오류 | UI의 `Y`, `GX`, `GY` 이름과 Controller MCD의 실제 축 이름 비교 |
...
| --- | --- | --- |
| 목적 | Stage PositionFeedback wait 해제와 GX/GY 순서 검증 | 실제 Scanner 좌표 이동 골격 검증 |
...
Virtual 환경에서도 `Y`, `GX`, `GY`가 MCD에 정의되어 있어야 한다. 생성식은 AeroScript 토큰 구분을 위해 `$StartYPos + 40`, `$StartYPos - 10`처럼 연산자 앞뒤에 공백을 넣는다.
