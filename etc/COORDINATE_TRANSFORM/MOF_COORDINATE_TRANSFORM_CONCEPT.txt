...
| Scanner | FirstScannerInitialStageX/Y | 예: 115, 2200 | H1 Scanner의 실제 Stage 초기 중심 |
| Calibration | ScannerOriginTolerance | 0.001 | H1 초기위치와 물리 Offset 기대값 허용오차 |
...
프로그램에서는 H1 Scanner 초기 위치를 별도 입력으로 보관하고 다음 식을 검증한다.

```text
ExpectedH1 = ReviewCenter + ReviewToFirstScannerOffset
OriginError = FirstScannerInitialStage - ExpectedH1
OriginValid = abs(OriginError.X/Y) <= ScannerOriginTolerance
```

Scanner 배치는 `FirstScannerInitialStage`를 실제 기준으로 사용하고, Physical Offset은 Review 좌표 변환에 사용한다. 두 값이 불일치하면 Process G와 Review 좌표가 서로 다른 장비 Geometry를 사용하게 되므로 실제 운전에서는 Plan 생성을 차단해야 한다.
...
| Review Camera Coordinate | 실제 Review Camera 중심 기준 좌표 + 선택 DOE Beam 위치 | `(x, y)` |
...
ScannerRelativeStage = ProcessStageTarget - SelectedHead.CenterStage
ReviewCoordinate     = ScannerRelativeStage
                     + CameraToSelectedHeadPhysicalOffset
                     + BeamOffsetStage

검증식:
ReviewCoordinate     = ProcessStageTarget - ReviewCenterStage
                     + BeamOffsetStage
```

따라서 사용자가 `Scanner #2`와 `DOE Beam #5`를 선택하면, H2의 Scanner 상대 Stage 좌표를 물리 Offset으로 Review Camera 좌표계에 되돌린 뒤 DOE05의 실제 Stage 방향 위치를 더한다. 각 Row에는 실제 가공을 담당하는 `Process Scanner`가 함께 표시되므로 Scanner Head별 구분도 가능하다.
